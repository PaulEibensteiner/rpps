globals
[eps step_size tilestate]

breed [bouncer-particles bouncer-particle]
breed [rotation-particles rotation-particle]
breed [signal-particles signal-particle]
breed [radius-particles radius-particle]

; PRIMORDIAL PARTICLE SYSTEM DEFINITION BEGINNING
...
to set-breeds
  ask turtles [
    ...
    (ifelse
      N = 7 [
        set breed signal-particles
        set color green
      ]
      N = 3 [
        set breed bouncer-particles
        set color blue
      ]
      N = 0 or N = 2 or N = 4 or N = 6 [
        set breed rotation-particles
        set color red
      ]
      [set color gray]
    )
...
; PRIMORDIAL PARTICLE SYSTEM DEFINITION END
...
to create-top-side[x y d transform]
  ...
  ; create-signal (- sqrt 50) (d + 20 - sqrt 50) 135
...
to create-bottom-side [x y d transform]
  set d d - 20
  ;create-signal 0 (- d - 10) 0
  create-rotation-transformed 0 (- d - 10) 20 1 transform x y
  ; create-signal (sqrt 50) (- d - 10 - sqrt 50) 0
...
; Color all tiles
to color-tiles
  let unnecessary_state true
  ask bouncer-particles [
    let margin 3
    let k 200
    ; if (abs ycor) > 100 [ print (word "centerx: " xcor ", " ycor)]
    (ifelse
      ...
      check_multiple_of_k (xcor - sqrt 50) (ycor + sqrt 50) margin k [
        if tilestate [
          hatch-radius-particles 1 [
            set shape "circle"
            set size radius * 2
            set color [39 52 139 25]
            stamp
            die
          ]
          stamp
        ]
        set unnecessary_state false
    ]
...
to show-radii
  ask turtles [
    let c 0
    (ifelse
      N = 7 [set c [0 152 58 25]]
      N = 3 [set c [39 52 139 25]]
        [set c [227 6 19 25]]
    )
    hatch-radius-particles 1 [
      set shape "circle"
...
to show-signal-radius
  ask signal-particles [
    hatch-radius-particles 1 [
      set shape "circle"
      set size radius * 2
      set color [0 152 58 12]
      stamp
      die
    ]
    stamp
